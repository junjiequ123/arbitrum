...
void ArbCore::operator()() {
    ...
    while (!arbcore_abort) {
        ...
        if (current_seconds >= next_rocksdb_save_timestamp) {
            ...
            auto status = tx.createCheckpoint(checkpoint_dir.string());
            if (!status.ok()) {
                std::cerr << "Unable to save checkpoint into " << checkpoint_dir
                          << std::endl;
            } else {
                auto save_elapsed = seconds_since_epoch() - current_seconds;
                std::cerr << "Saving rocksdb checkpoint took " << save_elapsed
                          << " seconds" << std::endl;
            }
